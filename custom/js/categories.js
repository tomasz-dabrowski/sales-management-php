...
$(document).ready(function() {
    ...
    // manage categorys table
	manageCategoriesTable = $('#manageCategoriesTable').DataTable({
		'ajax' : 'controller/fetchCategories.php',
		'order': []
	});

	// submit categories form modal
...
function editCategories(categoriesId = null) {
	if(categoriesId) {
		...
		$.ajax({
			url: 'controller/fetchSelectedCategories.php',
			type: 'post',
	...
	$.ajax({
		...
		success:function(response) {			
			...
			$("#removeCategoriesBtn").unbind('click').bind('click', function() {
				...
				$.ajax({
					url: 'controller/removeCategories.php',
					type: 'post',
					data: {categoriesId: categoriesId},
					dataType: 'json',
					success:function(response) {
