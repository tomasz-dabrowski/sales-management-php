...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger">Client Name is required</p>');
			...
			if(clientContact == "") {
				$("#clientContact").after('<p class="text-danger">Contact Number is required </p>');
			...
			if(discount == "") {
				$("#discount").after('<p class="text-danger">Discount field is required</p>');
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
			...
			if(clientContact == "") {
				$("#clientContact").after('<p class="text-danger">Contact number field is required </p>');
				$('#clientContact').closest('.form-group').addClass('has-error');
			} else {
				$('#clientContact').closest('.form-group').addClass('has-success');
			} // /else
...
function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			url: 'controller/fetchOrderData.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'json',
			success:function(response) {				
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount && paymentType && paymentStatus) {
						...
						$.ajax({
							url: 'controller/editPayment.php',
							type: 'post',
							data: {
								orderId: orderId,
								payAmount: payAmount,
